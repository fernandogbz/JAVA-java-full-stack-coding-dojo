...
public class ControladorDirectores {
	...
	public String procesaRegistro(@Valid @ModelAttribute("director") Director director,
		...
		if(result.hasErrors()) {
			...
			Director nuevoDirector = this.servicioDirectores.agregarDirector(director);
			sesion.setAttribute("correo", nuevoDirector.getCorreo());
			sesion.setAttribute("nombre", nuevoDirector.getNombre() + " " + nuevoDirector.getApellido());
			sesion.setAttribute("id", nuevoDirector.getId());
